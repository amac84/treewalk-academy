      ...
      <div className="player-layout">
        <article className="video-shell">
          ...
          <section className="stack-sm">
            ...
            {activeSegment?.transcriptStatus === 'processing' ? (
              <p className="muted">Transcript is generating. Check back shortly.</p>
            ) : null}
            {activeSegment?.transcriptStatus === 'error' ? (
              <p className="inline-error">
                Transcript unavailable: {activeSegment.transcriptErrorMessage ?? 'Generation failed.'}
              </p>
            ) : null}
            {activeSegment?.transcriptText ? (
              <div className="video-placeholder">
                <p>{activeSegment.transcriptText}</p>
              </div>
            ) : null}
            {!activeSegment?.transcriptText &&
            activeSegment?.transcriptStatus !== 'processing' &&
            activeSegment?.transcriptStatus !== 'error' ? (
              <p className="muted">Transcript not available yet for this segment.</p>
            ) : null}
          </section>
        </article>
